...
def process_query(user_query):
    ...
    st.session_state.messages.append({"role": "Control", "content": user_emotions})

    with st.chat_message("assistant"):
        with st.spinner("Rephrasing the answer..."):
            rephrased_output = rephrase_sprinklr(answer1, user_query, tactic_prompt)
            prompts = db.get_prompts()
            
    st.session_state.messages.append({"role": "assistant", "content": rephrased_output})
