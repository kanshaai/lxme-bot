...
def conversation_history(messages):
    ...
    history = ''
    for message in messages:
        if message['role'] == 'Control' or message['role'] == 'Prompt':
            continue
        history += f"{message['role']}: {message['content']}\n"
    return history
...
def process_query(user_query, original_response):
    ...
    st.session_state.messages.append({"role": "user", "content": original_response})
    ...
    with st.chat_message("assistant"):
        with st.spinner("Rephrasing the answer..."):
                ...
                try:
                    ...
                    return
                final_prompt = prompt.replace("{{user_query}}", user_query).replace("{{original_response}}", original_response).replace("{{conversation}}", conversation_history(st.session_state.messages))
...
- If you have acknowledged user's emotions once in the conversation history, no need to do it again and again

'''
                st.session_state.chat_prompt = final_prompt
                st.session_state.messages.append({"role": "Prompt", "content":st.session_state.chat_prompt})
...
def render_chat_playground():
    ...
    # Create two input fields and a submit button
    with st.form(key='chat_form'):
        user_input = st.text_input("Enter User's response:")
        original_response = st.text_input("Enter original response:")
        submit_button = st.form_submit_button(label='Submit')

    # Process the inputs when the form is submitted
    if submit_button and user_input and original_response:
        process_query(user_input, original_response)